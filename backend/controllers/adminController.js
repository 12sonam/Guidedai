...
//         if (status === 'declined' && !feedback) {
//             return res.status(400).json({
//                 success: false,
//                 message: "Feedback is required when declining an itinerary"
//             });
//         }

//         itinerary.status = status;
//         itinerary.feedback = feedback || null; // Store feedback if provided
//         await itinerary.save();

//         // Send email notification if itinerary is declined
//         if (status === 'declined') {
//             const emailSubject = 'Your Itinerary Has Been Declined';
//             const emailMessage = `
// Dear ${itinerary.fullName || 'Traveler'},

// Your itinerary "${itinerary.tourName}" has been declined by our team. Below is the feedback provided to help you revise and resubmit your itinerary:

// Feedback: ${feedback}

// Please update your itinerary based on the feedback and resubmit it via your Itinerary form. If you have any questions, feel free to contact us.

// Best regards,
// The GuideDai Team
//             `;

//             console.log(`Attempting to send email to: ${itinerary.email}`); // Log recipient email
//             try {
...
//                 });
//                 console.log(`Email sent successfully to ${itinerary.email}: Message ID ${emailResult.messageId}`);
//             } catch (emailError) {
//                 console.error(`Failed to send email to ${itinerary.email}:`, emailError);
//                 // Include email error in response but don't fail the request
//                 return res.status(200).json({
//                     success: true,
//                     message: `Itinerary ${status} successfully, but failed to send notification email`,
//                     data: itinerary,
...
The GuideDai Team
      ...
      const emailSubject = 'Your Itinerary Has Been Accepted';
      const emailMessage = `
Dear ${itinerary.fullName || 'Traveler'},

We are pleased to inform you that your itinerary "${itinerary.tourName}" has been accepted by our team. We will contact you shortly to proceed with the next steps.

Thank you for choosing GuideDai!
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    // Calculate total revenue (only for completed bookings, convert to USD conditionally)
    const revenueResult = await Booking.aggregate([
      { $match: { payment: "completed" } },
      {
        $group: {
          ...
          total: {
            $sum: {
              $cond: [
                { $eq: ['$originalCurrency', 'USD'] }, // If currency is USD, use originalPrice as is
                '$originalPrice',
                { $divide: ['$originalPrice', EXCHANGE_RATE] } // Convert NPR to USD if not USD
              ]
            }
          }
        }
    ...
    const totalRevenueUSD = revenueResult.length > 0 ? revenueResult[0].total : 0;
    ...
    // Revenue by Tour (only for completed bookings, convert to USD conditionally)
    const revenueByTourResult = await Booking.aggregate([
      { $match: { payment: "completed" } },
      {
        $group: {
      ...
      { $unwind: '$tour' },
      {
        $project: {
          name: '$tour.title',
          value: '$totalRevenue' // Already in USD due to conditional conversion
        }
      },
      { $sort: { value: -1 } },
      { $limit: 5 }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        totalBookings: bookings,
        totalReviews: reviews,
        totalRevenue: totalRevenueUSD, // Return revenue in USD
        avgRating: avgRating,
        revenueByTour: revenueByTourResult,
        bookingStatus: bookingStatusResult
      }
